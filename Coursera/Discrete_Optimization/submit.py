...
def check_login(metadata, login, password):
    ...
    (result, string) = submit_solution(metadata.url, login,
                                       ch_resp, sid, submission, source, state, ch_aux)
...
def submit():
    ...
    for part in selected_parts:
        (login, ch, state, ch_aux) = get_challenge(
            metadata.url, login, part.sid)
        if not login or not ch or not state:
            print '\n!! Error: %s\n' % login
            return
        ...
        ch_resp = challenge_response(login, password, ch)
        (result, string) = submit_solution(metadata.url, login, ch_resp,
                                           part.sid, submission, get_source(part.source), state, ch_aux)
...
def basic_prompt():
    ...
    password = raw_input(
        'Submission Password (from the programming assignments page. This is NOT your own account\'s password): ')
    return (login, password)
...
def part_prompt(parts):
    ...
    part_text = raw_input(
        'Please enter which part(s) you want to submit (0-' + str(len(parts)) + '): ')
    selected_parts = []
...
def submit_solution(c_url, email_address, ch_resp, sid,
                    output, source, state, ch_aux):
